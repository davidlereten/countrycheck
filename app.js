// CountryCheck hard-fixed Supabase connection. Do not use env-style values here.
...
function ensureSupabase(){
  ...
  if(!projectUrl || !publishableKey){
    setMessage('loginMessage', 'Supabase config is missing.');
    return false;
  }
  ...
  try {
    if(!sb) sb = window.supabase.createClient(projectUrl, publishableKey);
    return true;
  } catch (err) {
