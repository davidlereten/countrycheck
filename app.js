const URL = window.COUNTRYCHECK_SUPABASE_URL;
const KEY = window.COUNTRYCHECK_SUPABASE_ANON_KEY;
let sb = null;
...
function ensureSupabase(){
  if(!URL || !KEY || URL.includes('PASTE_') || KEY.includes('PASTE_')){
    qs('loginMessage').textContent = 'Supabase config is missing. Paste project URL and anon key into supabase-config.js.';
    return false;
  }
...
qs('loginForm').addEventListener('submit', async (e)=>{
  ...
  if(!/^[a-z]+\.[a-z]$/.test(username)){
    qs('loginMessage').textContent = 'Username format should be like paul.a';
    return;
  }
  const { data, error } = await sb.auth.signInWithPassword({ email: usernameToEmail(username), password });
  if(error){ qs('loginMessage').textContent = 'Login failed. Check name and password.'; return; }
  currentUser = data.user;
  if(password === defaultPasswordFor(username)){
    page('passwordPage');
    return;
  }
  page('appPage');
...
qs('passwordForm').addEventListener('submit', async (e)=>{
  ...
  if(error){ qs('passwordMessage').textContent = 'Password update failed. Try again.'; return; }
  page('appPage');
  await loadDatabase();
  bootApp();
});
