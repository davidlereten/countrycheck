// CountryCheck V5.4 - pro header, viewport lock, agent data via Edge Function, admin live tools.
...
function lockViewportAndBasicCopyProtection(){
  // UI deterrent only: real data security is enforced by Supabase RLS + Edge Functions.
  ...
  document.addEventListener('contextmenu', (e) => {
    if(!e.target.closest('input, textarea, select, .admin-table')) e.preventDefault();
  });
...
async function callCheckerApi(action, payload={}){
  const { data } = await supabaseClient.auth.getSession();
  const token = data?.session?.access_token;
  if(!token) throw new Error('No active session. Please login again.');
  const res = await fetch(`${SUPABASE_PROJECT_URL}/functions/v1/checker-api`, {
    method:'POST',
    headers:{
  ...
  if(!res.ok || body?.error) throw new Error(body?.error || `Checker API failed (${res.status})`);
  return body;
}

function renderRuleValue(v){
  if(v && typeof v === 'object' && v.country_code) return countryLine(v.country_code) + escapeHtml(v.suffix || '');
  if(String(v ?? '').includes('<span')) return String(v);
  return escapeHtml(v);
}
...
async function init(){
  lockViewportAndBasicCopyProtection();
  try{
...
async function loadData(){
  if(currentProfile?.role === 'agent'){
    const data = await callCheckerApi('bootstrap');
    countries = data.countries || [];
    documents = data.documents || [];
    rules = []; // Agents do not receive the rules database in the browser.
...
function renderDecision(d){
  ...
  $('decisionBox').innerHTML = `<div class="status"><span class="dot"></span><strong>${escapeHtml(d.status)}</strong></div><div class="reason">${escapeHtml(d.reason)}</div><div class="rule-card">${d.rows.map(([k,v])=>`<div class="rule-row"><div class="rule-key">${escapeHtml(k)}</div><div class="rule-value">${renderRuleValue(v)}</div></div>`).join('')}</div>`;
}
...
async function checkCountry(){
  ...
  if(currentProfile?.role === 'agent'){
    try{
      ...
      const res = await callCheckerApi('check', { input });
      renderDecision(res.decision);
      await updatePresence('checker', `Checked ${country(input.reg).name} / ${country(input.nat).name}`, res.history || null);
    }catch(err){
      renderDecision({status:'Check Failed', cls:'rejected', reason:err.message || 'Could not complete check.', rows:[['Action','Please contact admin.']]});
    }finally{
  ...
  const payload = {
    ...
    details: { rows: decision.rows.map(([k,v]) => [k, String(typeof v === 'object' ? JSON.stringify(v) : v).replace(/<[^>]+>/g,'')]) }
  };
